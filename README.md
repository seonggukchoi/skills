...
| [`herdr-enhanced`](skills/herdr-enhanced) | Judgment rules for the herdr terminal multiplexer — pane targeting, layout, lifecycle, and a request/reply protocol between agents. | `npx skills add seonggukchoi/skills --skill herdr-enhanced` |
...
Some skills rely on external tools — e.g. the `codex` CLI, the Bitwarden `bw` CLI, `pandoc`, a running WaterCrawl instance (for `watercrawl`), the `herdr` terminal multiplexer and its official `herdr` skill (for `herdr-enhanced`), Node.js 18+ and a CDP-reachable browser (for `agent-browser-human-control`), or Apple Silicon + `mlx-whisper` (for `youtube-markdown`). See each skill's `SKILL.md` for its prerequisites.
